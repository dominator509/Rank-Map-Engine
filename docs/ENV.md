...
| `BASE_PATH`                   | Optional build/runtime        | Vite base path for the RankMap and mockup frontend builds. Defaults to `/`.                                                | `/`                                                     |
| `API_SERVER_URL`              | Optional local/browser checks | API target used by the frontend Vite proxy during local development and browser/page-load runners.                         | `http://localhost:8080`                                 |
| `PUBLIC_APP_URL`              | Optional hosting alias        | Alternate public app URL accepted by deploy preflight when `APP_URL` is not set.                                           | `https://app.example.com`                               |
| `ALLOWED_ORIGINS`             | Optional API CORS             | Comma-separated extra browser origins allowed by the API server CORS guard.                                                | `https://staging.example.com`                           |
| `REPLIT_DOMAINS`              | Optional Replit hosting       | Comma-separated Replit-hosted domains allowed by the API server CORS guard.                                                | `example.replit.app`                                    |
| `LOG_LEVEL`                   | Optional                      | Server log level. Defaults to `info`. Use `silent` for focused local tests.                                                | `info`                                                  |
...
| `LIVE_SERVICES_OPTIONAL` / `ALLOW_NO_LIVE_SERVICE_CREDENTIALS` | Optional local or preflight dry run | Set `true` to allow `test:live:services` to exit 0 when no live provider credentials are configured. Keep unset for launch evidence so missing credentials fail closed. |
| `LIVE_KEYWORD_PROVIDER_CHECKS`                                 | Optional live diagnostics           | Set `true` to run quota-spending Ahrefs, Semrush, and DataForSEO live keyword checks when credentials are present.                                                      |
| `LIVE_KEYWORD_QUERY`                                           | Optional live diagnostics           | Query used by quota-spending keyword provider diagnostics. Defaults to `seo software`.                                                                                  |

## Local Testing

| Variable                      | Required When               | Description                                                                                               |
| ----------------------------- | --------------------------- | --------------------------------------------------------------------------------------------------------- |
| `PLAYWRIGHT_BASE_URL`         | Optional browser e2e        | Base URL used by Playwright when tests are invoked directly. The wrapper scripts set their own local URL. |
| `UPDATE_PLAYWRIGHT_SNAPSHOTS` | Optional browser e2e update | Set `1` only when intentionally regenerating Playwright screenshot snapshots.                             |
