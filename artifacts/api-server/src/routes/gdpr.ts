...
router.get("/gdpr/export", requireAuth, async (req, res): Promise<void> => {
  ...
  const [myComments, myAuditLogs, myNotifications, myKeywords, myBriefs] = await Promise.all([
    db
      .select()
      .from(commentsTable)
      .where(and(eq(commentsTable.userId, userId), eq(commentsTable.tenantId, tenantId))),
    db
      .select()
      .from(auditLogTable)
      .where(and(eq(auditLogTable.userId, userId), eq(auditLogTable.tenantId, tenantId))),
    db
      .select()
...
router.delete(
  "/gdpr/me",
  requireAuth,
  requireRole(["agency_admin", "agency_user", "client_viewer", "super_admin"]),
  async (req, res): Promise<void> => {
    ...
    if (user?.role === "agency_admin") {
      res.status(422).json({
        error:
          "Agency admins must transfer ownership before deleting their account. Contact support.",
      });
      return;
    }
