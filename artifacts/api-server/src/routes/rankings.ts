...
router.get("/projects/:projectId/rankings", requireAuth, async (req, res): Promise<void> => {
  ...
  const keywords = await db
    .select({ id: keywordsTable.id, phrase: keywordsTable.phrase, searchVolume: keywordsTable.searchVolume })
    .from(keywordsTable)
  ...
  const result = await Promise.all(
    keywords.map(async (kw) => {
      const [latest] = await db
        ...
        .where(and(eq(keywordRankingsTable.keywordId, kw.id), eq(keywordRankingsTable.tenantId, tenantId)))
        .orderBy(desc(keywordRankingsTable.checkedAt))
        .limit(1);
      return { ...kw, latestPosition: latest?.position ?? null, latestUrl: latest?.url ?? null, checkedAt: latest?.checkedAt ?? null };
    }),
  );
...
router.get("/projects/:projectId/rankings/:keywordId/history", requireAuth, async (req, res): Promise<void> => {
  const { tenantId } = req.session.user!;
  const projectId = parseInt(req.params.projectId as string, 10);
  const keywordId = parseInt(req.params.keywordId as string, 10);
  if (isNaN(projectId) || isNaN(keywordId)) { res.status(400).json({ error: "Invalid id" }); return; }
  if (!(await assertProjectAccess(projectId, tenantId))) { res.status(404).json({ error: "Project not found" }); return; }

  const history = await db
    .select()
    .from(keywordRankingsTable)
    .where(and(eq(keywordRankingsTable.keywordId, keywordId), eq(keywordRankingsTable.tenantId, tenantId)))
    .orderBy(desc(keywordRankingsTable.checkedAt))
    .limit(90);
...
router.post("/projects/:projectId/rankings/check", requireAuth, requireRole(["agency_admin", "agency_user", "super_admin"]), async (req, res): Promise<void> => {
  const { tenantId } = req.session.user!;
  const projectId = parseInt(req.params.projectId as string, 10);
  if (isNaN(projectId)) { res.status(400).json({ error: "Invalid projectId" }); return; }
  if (!(await assertProjectAccess(projectId, tenantId))) { res.status(404).json({ error: "Project not found" }); return; }
...
router.post("/projects/:projectId/rankings/check-all", requireAuth, requireRole(["agency_admin", "agency_user", "super_admin"]), async (req, res): Promise<void> => {
  const { tenantId } = req.session.user!;
  const projectId = parseInt(req.params.projectId as string, 10);
  if (isNaN(projectId)) { res.status(400).json({ error: "Invalid projectId" }); return; }
  if (!(await assertProjectAccess(projectId, tenantId))) { res.status(404).json({ error: "Project not found" }); return; }
  ...
  if (keywords.length === 0) { res.json({ checked: 0 }); return; }
