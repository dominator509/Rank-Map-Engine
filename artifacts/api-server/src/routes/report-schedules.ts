...
router.get("/projects/:projectId/report-schedules", requireAuth, async (req, res): Promise<void> => {
  const { tenantId } = req.session.user!;
  ...
  const schedules = await db
    .select()
    .from(reportSchedulesTable)
    .where(and(eq(reportSchedulesTable.projectId, projectId), eq(reportSchedulesTable.tenantId, tenantId)));
  res.json(schedules);
});

router.post("/report-schedules", requireAuth, requireRole(["agency_admin", "super_admin"]), async (req, res): Promise<void> => {
  const { tenantId } = req.session.user!;
...
router.patch("/report-schedules/:id", requireAuth, requireRole(["agency_admin", "super_admin"]), async (req, res): Promise<void> => {
  const { tenantId } = req.session.user!;
...
router.delete("/report-schedules/:id", requireAuth, requireRole(["agency_admin", "super_admin"]), async (req, res): Promise<void> => {
  const { tenantId } = req.session.user!;
  const id = parseInt(req.params.id as string, 10);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid id" }); return; }
