...
interface Overview {
  totals: {
    clients: number; projects: number; keywords: number; clusters: number;
    briefs: number; reports: number; aiTasksThisMonth: number;
  };
...
interface ProjectStat {
  id: number; name: string; keywordCount: number; clusterCount: number; briefCount: number;
}

interface VelocityRow { day: string; count: number; }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-8">
      <div className="max-w-6xl mx-auto space-y-8">
        <div>
          <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2"><BarChart2 className="w-7 h-7" />Analytics</h1>
          <p className="text-muted-foreground mt-1">Workspace-wide performance overview</p>
        </div>

        {/* Totals */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {overviewLoading ? Array.from({ length: 7 }).map((_, i) => <Skeleton key={i} className="h-28 rounded-xl" />) : [
            ...
            <Card key={label}>
              ...
              <CardContent><div className="text-3xl font-bold">{value ?? 0}</div></CardContent>
            </Card>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="md:col-span-2">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><TrendingUp className="w-5 h-5" />Keyword Velocity (30 days)</CardTitle>
              <CardDescription>New keywords added per day</CardDescription>
            </CardHeader>
            <CardContent>
              {(velocity as VelocityRow[]).length === 0 ? (
                <p className="text-muted-foreground text-sm py-8 text-center">No keywords added in the last 30 days.</p>
              ) : (
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {overviewLoading ? <Skeleton className="h-24" /> : (overview?.briefsByStatus ?? []).map((b) => (
                <div key={b.status} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${STATUS_COLORS[b.status] ?? "bg-gray-400"}`} />
        ...
        <Card>
          ...
          <CardContent>
            {projLoading ? <Skeleton className="h-40" /> : (projects as ProjectStat[]).length === 0 ? (
