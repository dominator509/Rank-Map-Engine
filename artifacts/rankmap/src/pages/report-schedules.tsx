...
export default function ReportSchedules() {
  ...
  const create = useMutation({
    ...
    onError: (e: Error) =>
      toast({ title: e.message || "Failed to create schedule", variant: "destructive" }),
  });

  const toggle = useMutation({
    mutationFn: ({ id, isActive }: { id: number; isActive: boolean }) =>
      customFetch(`/api/report-schedules/${id}`, {
        method: "PATCH",
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Automate report delivery to clients and stakeholders
            </p>
          </div>
            ...
            <DialogContent>
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    placeholder="client@example.com, boss@agency.com"
                    value={emails}
                ...
                <Button
                  ...
                  disabled={!projectId || !emails || create.isPending}
                >
                  Create Schedule
                </Button>
              </div>
              ...
              <Card key={s.id}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-base">
                          {projectMap[s.projectId] ?? `Project ${s.projectId}`} —{" "}
                          {TYPE_LABELS[s.reportType] ?? s.reportType}
                        </CardTitle>
                ...
                <CardContent className="pt-0">
                  <p className="text-xs text-muted-foreground">
                    {s.lastSentAt
                      ? `Last sent: ${format(new Date(s.lastSentAt), "MMM d, yyyy HH:mm")}`
                      : "Never sent"}{" "}
