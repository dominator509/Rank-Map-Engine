...
import { BarChart2, RefreshCw, TrendingDown, TrendingUp, Minus, Loader2 } from "lucide-react";
...
interface RankedKeyword {
  id: number; phrase: string; searchVolume?: number;
  latestPosition: number | null; latestUrl: string | null; checkedAt: string | null;
}

function PositionBadge({ pos }: { pos: number | null }) {
  if (pos === null) return <Badge variant="outline" className="text-muted-foreground">Not tracked</Badge>;
  ...
  if (pos <= 30) return <Badge className="bg-yellow-500 hover:bg-yellow-600 text-white">#{pos}</Badge>;
...
export default function Rankings() {
  ...
  const checkAll = useMutation({
    mutationFn: () => customFetch(`/api/projects/${projectId}/rankings/check-all`, { method: "POST" }),
    ...
    <div className="flex-1 overflow-y-auto p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2"><BarChart2 className="w-7 h-7" />Rank Tracking</h1>
        ...
        <div className="flex items-center gap-4">
          ...
          {projectId && (
            <Button onClick={() => checkAll.mutate()} disabled={checkAll.isPending}>
              {checkAll.isPending ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <RefreshCw className="w-4 h-4 mr-2" />}
              Check All Rankings
            </Button>
          )}
        </div>
        ...
        {projectId && (
          <>
            <div className="grid grid-cols-3 gap-4">
              <Card><CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Top 10</CardTitle></CardHeader><CardContent><div className="text-3xl font-bold text-green-600">{top10}</div></CardContent></Card>
              <Card><CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Top 30</CardTitle></CardHeader><CardContent><div className="text-3xl font-bold text-yellow-600">{top30}</div></CardContent></Card>
              <Card><CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Total Tracked</CardTitle></CardHeader><CardContent><div className="text-3xl font-bold">{ranked.length}</div></CardContent></Card>
            </div>
                    ...
                    <TableBody>
                      {(rankings as RankedKeyword[]).map((r) => (
                        <TableRow key={r.id}>
                          ...
                          <TableCell><PositionBadge pos={r.latestPosition} /></TableCell>
                          <TableCell className="max-w-xs truncate text-xs text-muted-foreground">{r.latestUrl ?? "—"}</TableCell>
