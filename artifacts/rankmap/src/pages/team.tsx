...
export default function Team() {
  ...
  const removeMember = useMutation({
    mutationFn: (userId: number) =>
      customFetch(`/api/team/${userId}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["team"] });
  ...
  const sendInvite = useMutation({
    mutationFn: () =>
      customFetch("/api/team/invite", { method: "POST", body: JSON.stringify({ email: inviteEmail, role: inviteRole }) }),
    ...
    <div className="flex-1 overflow-y-auto p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={isInviteOpen} onOpenChange={(o) => { setIsInviteOpen(o); if (!o) { setInviteLink(null); setInviteEmail(""); } }}>
            ...
            <DialogContent>
              ...
              {inviteLink ? (
                <div className="space-y-4">
                  ...
                  <div className="bg-muted rounded-md p-3 text-xs font-mono break-all">{inviteLink}</div>
                  <Button onClick={() => { navigator.clipboard.writeText(inviteLink); toast({ title: "Copied!" }); }} variant="outline" className="w-full">Copy Link</Button>
                  <Button onClick={() => { setIsInviteOpen(false); setInviteLink(null); }} className="w-full">Done</Button>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <Label>Email address</Label>
                    <Input placeholder="colleague@example.com" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} type="email" />
                  </div>
                  <div className="space-y-1.5">
                    <Label>Role</Label>
                  ...
                  <DialogFooter>
                    <Button onClick={() => sendInvite.mutate()} disabled={!inviteEmail || sendInvite.isPending} className="w-full">
            ...
            {isLoading ? (
              Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="flex items-center gap-3 py-4">
                  ...
                  <div className="space-y-1.5 flex-1"><Skeleton className="h-4 w-40" /><Skeleton className="h-3 w-24" /></div>
                </div>
              ))
            ) : members?.map((m) => (
              <div key={m.id} className="flex items-center gap-3 py-3">
                ...
                <Badge variant={ROLE_COLORS[m.role] ?? "secondary"}>{ROLE_LABELS[m.role] ?? m.role}</Badge>
                {m.lastLoginAt && (
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => updateRole.mutate({ userId: m.id, role: "agency_admin" })}>
                      <Shield className="w-4 h-4 mr-2" /> Make Admin
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => updateRole.mutate({ userId: m.id, role: "agency_user" })}>
                      Make Member
                    </DropdownMenuItem>
                    <DropdownMenuItem className="text-destructive" onClick={() => { if (confirm(`Remove ${m.fullName}?`)) removeMember.mutate(m.id); }}>
            ...
            <CardContent className="divide-y">
              {pending.map((inv) => (
                <div key={inv.id} className="flex items-center gap-3 py-3">
                  ...
                  <Button variant="ghost" size="sm" className="text-destructive h-8 w-8 p-0" onClick={() => cancelInvite.mutate(inv.id)}>
