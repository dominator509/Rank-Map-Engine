...
export default async function runPhase3(baseUrl) {
  ...
  const createdClient = await api.request("/api/clients", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: "Workflow Client" }) });
  ...
  const out = { phase: 3, baseUrl, steps: { register, me, createdClient, getClient } };
