...
async function ensureDockerAvailable() {
  ...
  throw new Error(
    [
      "Docker is required for the performance baseline because it starts a disposable Postgres container.",
      "Start Docker Desktop or make the Docker daemon reachable, then rerun `corepack pnpm run perf:baseline`.",
      output,
    ]
      .filter(Boolean)
      .join("\n"),
